...
void main() async {
  ...
  print(
      'Found ${directiveIssues.values.fold(0, (sum, list) => sum + list.length)} directives_ordering issues');
  ...
  for (final entry in directiveIssues.entries) {
    ...
    final dartImports = imports
        .where((i) => i.contains("'dart:") || i.contains('"dart:'))
        .toList()
      ..sort();
    final packageImports = imports
        .where((i) => i.contains("'package:") || i.contains('"package:'))
        .toList()
      ..sort();
    final relativeImports = imports
        .where((i) => !dartImports.contains(i) && !packageImports.contains(i))
        .toList()
      ..sort();
