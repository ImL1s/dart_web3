...
void main() {
  test('Verify connection to official Relay server', () async {
    ...
    try {
      ...
      if (error.contains('not upgraded to WebSocket') ||
          error.contains('WebSocketException')) {
        print(
            'Verified: Server reachable but rejected handshake (Expected with dummy Project ID).');
        return;
      }
