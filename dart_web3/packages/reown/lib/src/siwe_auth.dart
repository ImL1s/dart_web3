...
class SiweAuth {
  ...
  bool _verifyEcdsaSignature(Uint8List message, Uint8List signature, String address) {
...
class SiweMessage {
  ...
  factory SiweMessage.parse(String message) {
    ...
    return SiweMessage(
      ...
      nonce: fields['Nonce'] ?? '',
      issuedAt: DateTime.parse(fields['Issued At'] ?? DateTime.now().toIso8601String()),
...
class SiweConfig {
  ...
  SiweConfig({
    required this.domain,
    required this.uri, required this.chainId, this.statement,
    this.expirationTime,
    this.notBefore,
    this.requestId,
    this.resources,
...
class SiweAuthResult {
  ...
  SiweAuthResult({
    required this.siweMessage, this.proposal,
    this.session,
    this.signature,
  ...
  Future<SiweAuthResult> authenticate({
    ...
    try {
      ...
      final authResult = await siweAuth.authenticateWithSiwe(
        ...
        customMessage: result.siweMessage.address.isEmpty 
            ? result.siweMessage
            : null,
      );
