...
class LightAccount extends BaseSmartAccount {
  ...
  Future<bool> supportsEIP1271() async {
    try {
      ...
      final callData =
          _encodeSupportsInterfaceCall('0x1626ba7e'); // EIP-1271 interface ID
  ...
  String _encodeSupportsInterfaceCall(String interfaceId) {
    ...
    final paddedInterfaceId =
        interfaceId.replaceFirst('0x', '').padLeft(64, '0');
  ...
  String _encodeIsValidSignatureCall(String hash, String signature) {
    ...
    final paddedSigData =
        sigData.padRight(((sigData.length + 63) ~/ 64) * 64, '0');
