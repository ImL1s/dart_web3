    ...
    for (var i = 0; i < vectors.length; i++) {
        ...
        test('Vector #$i: $description', () {
            ...
            final inputs = inputsSrc.map((inData) {
                ...
                final witness = witnessSrc?.map((w) => HexUtils.decode(w as String)).toList();
            ...
            final outputs = outputsSrc.map((outData) {
                ...
                final scriptPubKey = HexUtils.decode(outMap['scriptPubKey'] as String);
            ...
            final actualHex = HexUtils.encode(tx.toBytes(segwit: isSegwit), prefix: false);
            
            expect(actualHex, equals(expectedHex), reason: 'Serialization mismatch');
        });
    }
  });
}
