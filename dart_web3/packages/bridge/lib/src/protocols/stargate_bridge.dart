...
class StargateBridge extends BridgeProtocol {
  ...
  Future<BridgeQuote?> getQuote(BridgeParams params) async {
    ...
    try {
      ...
      final body = {
        ...
        'dstPoolId': await _getPoolId(params.toToken, params.destinationChainId),
  ...
  Future<bool> isTokenPairSupported(
    ...
    try {
      ...
      final supportedSymbols = ['USDC', 'USDT', 'BUSD', 'DAI', 'FRAX', 'ETH', 'WETH'];
  ...
  Future<BridgeFeeBreakdown> getFeeBreakdown(BridgeParams params) async {
    ...
    final protocolFee = params.amount * BigInt.from(6) ~/ BigInt.from(10000); // 0.06%
    final gasFee = BigInt.from(30000000000000000); // ~0.03 ETH equivalent
    final liquidityFee = params.amount * BigInt.from(1) ~/ BigInt.from(10000); // 0.01%
  ...
  BridgeQuote _parseQuoteResponse(Map<String, dynamic> data, BridgeParams params) {
    ...
    final protocolFee = BigInt.parse(feeData['protocolFeeLD'] as String? ?? '0');
    final liquidityFee = BigInt.parse(feeData['liquidityFeeLD'] as String? ?? '0');
    ...
    final limits = BridgeLimits(
      minAmount: BigInt.parse(limitsData['minAmountLD'] as String? ?? '1000000'),
      maxAmount: BigInt.parse(limitsData['maxAmountLD'] as String? ?? '10000000000'),
      dailyLimit: BigInt.parse(limitsData['dailyLimitLD'] as String? ?? '100000000000'),
      remainingDailyLimit: BigInt.parse(limitsData['remainingDailyLimitLD'] as String? ?? '100000000000'),
    );
    ...
    return BridgeQuote(
      ...
      validUntil: const Duration(minutes: 5), // Stargate quotes valid for 5 minutes
      metadata: {
        'stargateData': data,
        'poolIds': {
