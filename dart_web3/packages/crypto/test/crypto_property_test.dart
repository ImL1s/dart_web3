  ...
  group('Crypto Module Property Tests', () {
    ...
    test('Property 21: Keccak-256 Hash Consistency', () {
      ...
      for (var i = 0; i < 100; i++) {
        ...
        expect(
          _uint8ListEquals(hash1, hash2),
          isTrue,
          reason: 'Keccak-256 should be deterministic',
        );
        expect(
          _uint8ListEquals(hash2, hash3),
          isTrue,
          reason: 'Keccak-256 should be deterministic',
        );
        ...
        if (input.length > 1) {
          ...
          expect(
            _uint8ListEquals(hash1, differentHash),
            isFalse,
    ...
    test('Property 22: BIP-39 Mnemonic Validation', () {
      ...
      for (var i = 0; i < 50; i++) {
        ...
        try {
          ...
          expect(
            _uint8ListEquals(seed, seedWithPassphrase),
            isFalse,
    ...
    test('Property 23: HD Wallet Derivation Consistency', () {
      ...
      for (var i = 0; i < 50; i++) {
        ...
        try {
          ...
          expect(
            _uint8ListEquals(
                derived1.getPrivateKey(), derived2.getPrivateKey()),
            isTrue,
            reason: 'Same derivation path should produce same private key',
          );

          expect(
            _uint8ListEquals(derived1.getPublicKey(), derived2.getPublicKey()),
            isTrue,
            reason: 'Same derivation path should produce same public key',
          );

          expect(
            derived1.getAddress().toString(),
            equals(derived2.getAddress().toString()),
          ...
          expect(
            _uint8ListEquals(
                derived1.getPrivateKey(), derived3.getPrivateKey()),
            isFalse,
          ...
          expect(
            _uint8ListEquals(child0.getPrivateKey(), child1.getPrivateKey()),
            isFalse,
