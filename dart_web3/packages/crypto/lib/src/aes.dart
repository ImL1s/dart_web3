import 'dart:typed_data';

/// Pure Dart implementation of AES-128 block cipher and CTR mode.
class AES {
  static const int blockSize = 16;

  // S-Box
  static const List<int> _sBox = [
    
  ];

  static const List<int> _rCon = [
    0x00,
    0x01,
    0x02,
    0x04,
    0x08,
    0x10,
    0x20,
    0x40,
    0x80,
    0x1b,
    0x36
  ];

  final Uint32List _roundKeys;

  AES(Uint8List key) : _roundKeys = _expandKey(key) {
    if (key.length != 16) {
      throw ArgumentError(
          'Only AES-128 (16-byte key) is supported for V3 Keystore');
    }
  }

  /// Encrypts data in CTR mode.
  Uint8List ctr(Uint8List data, Uint8List iv) {
    final result = Uint8List(data.length);
    final counter = Uint8List.fromList(iv);
    final keystream = Uint8List(blockSize);

    for (var i = 0; i < data.length; i += blockSize) {
      _encryptBlock(counter, keystream);

      // XOR keystream with data
      final len = (i + blockSize > data.length) ? data.length - i : blockSize;
      for (var j = 0; j < len; j++) {
        result[i + j] = data[i + j] ^ keystream[j];
      }

      // Increment counter (big-endian)
      for (var j = blockSize - 1; j >= 0; j--) {
        counter[j] = (counter[j] + 1) & 0xFF;
        if (counter[j] != 0) break;
      }
    }

    return result;
  }

  void _encryptBlock(Uint8List input, Uint8List output) {
    var state = Uint8List.fromList(input);

    _addRoundKey(state, 0);

    for (var round = 1; round < 10; round++) {
      _subBytes(state);
      _shiftRows(state);
      _mixColumns(state);
      _addRoundKey(state, round);
    }

    _subBytes(state);
    _shiftRows(state);
    _addRoundKey(state, 10);

    output.setAll(0, state);
  }

  void _subBytes(Uint8List state) {
    for (var i = 0; i < blockSize; i++) {
      state[i] = _sBox[state[i]];
    }
  }

  void _shiftRows(Uint8List state) {
    final tmp = Uint8List.fromList(state);
    state[1] = tmp[5];
    state[5] = tmp[9];
    state[9] = tmp[13];
    state[13] = tmp[1];
    state[2] = tmp[10];
    state[6] = tmp[14];
    state[10] = tmp[2];
    state[14] = tmp[6];
    state[3] = tmp[15];
    state[7] = tmp[3];
    state[11] = tmp[7];
    state[15] = tmp[11];
  }

  void _mixColumns(Uint8List state) {
    for (var i = 0; i < 4; i++) {
      final a = state[i * 4];
      final b = state[i * 4 + 1];
      final c = state[i * 4 + 2];
      final d = state[i * 4 + 3];

      state[i * 4] = _mul2(a) ^ _mul3(b) ^ c ^ d;
      state[i * 4 + 1] = a ^ _mul2(b) ^ _mul3(c) ^ d;
      state[i * 4 + 2] = a ^ b ^ _mul2(c) ^ _mul3(d);
      state[i * 4 + 3] = _mul3(a) ^ b ^ c ^ _mul2(d);
    }
  }

  static int _mul2(int b) =>
      (b & 0x80) != 0 ? ((b << 1) ^ 0x1b) & 0xFF : (b << 1) & 0xFF;
  static int _mul3(int b) => _mul2(b) ^ b;

  void _addRoundKey(Uint8List state, int round) {
    for (var i = 0; i < 4; i++) {
      final word = _roundKeys[round * 4 + i];
      state[i * 4] ^= (word >> 24) & 0xFF;
      state[i * 4 + 1] ^= (word >> 16) & 0xFF;
      state[i * 4 + 2] ^= (word >> 8) & 0xFF;
      state[i * 4 + 3] ^= word & 0xFF;
    }
  }

  static Uint32List _expandKey(Uint8List key) {
    final w = Uint32List(44);
    for (var i = 0; i < 4; i++) {
      w[i] = (key[i * 4] << 24) |
          (key[i * 4 + 1] << 16) |
          (key[i * 4 + 2] << 8) |
          key[i * 4 + 3];
    }

    for (var i = 4; i < 44; i++) {
      var tmp = w[i - 1];
      if (i % 4 == 0) {
        // RotWord
        tmp = ((tmp << 8) & 0xFFFFFFFF) | (tmp >>> 24);
        // SubWord
        tmp = (_sBox[(tmp >> 24) & 0xFF] << 24) |
            (_sBox[(tmp >> 16) & 0xFF] << 16) |
            (_sBox[(tmp >> 8) & 0xFF] << 8) |
            _sBox[tmp & 0xFF];
        // XOR Rcon
        tmp ^= (_rCon[i ~/ 4] << 24);
      }
      w[i] = w[i - 4] ^ tmp;
    }
    return w;
  }
}
