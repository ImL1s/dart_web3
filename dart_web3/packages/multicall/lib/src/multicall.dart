...
class Multicall {
  ...
  Future<List<CallResult>> tryAggregate(List<Call> calls, {bool requireSuccess = false}) async {
  ...
  Future<MulticallBlockResult> aggregateWithBlock(List<Call> calls) async {
    if (_version != MulticallVersion.v3) {
      throw UnsupportedError('aggregateWithBlock is only supported in Multicall v3');
    }
  ...
  MulticallBlockResult _decodeAggregateWithBlockResult(Uint8List data, int callCount) {
    var offset = 0;
