...
class LedgerClient {
  ...
  final StreamController<LedgerConnectionState> _stateController =
  ...
  bool get isReady =>
      _state == LedgerConnectionState.connected && _appConfig != null;
  ...
  Future<LedgerAccount> getAccount(String derivationPath,
      {bool display = false}) async {
    _ensureConnected();

    try {
      final command =
          EthereumAPDU.getPublicKey(derivationPath, display: display);
  ...
  Future<LedgerSignResponse> signTransaction(
    ...
    try {
      ...
      final command =
          EthereumAPDU.signTransactionFirst(derivationPath, transactionData);
      final response = await _transport.exchange(command);
  ...
  Future<LedgerSignResponse> signPersonalMessage(
    ...
    try {
      final command =
          EthereumAPDU.signPersonalMessage(derivationPath, messageData);
      final response = await _transport.exchange(command);
  ...
  Future<LedgerSignResponse> signTypedData(
    ...
    try {
      final command =
          EthereumAPDU.signTypedData(derivationPath, domainHash, messageHash);
      final response = await _transport.exchange(command);
