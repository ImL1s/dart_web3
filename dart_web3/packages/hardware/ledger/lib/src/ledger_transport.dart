...
class LedgerUSBTransport implements LedgerTransport {
  ...
  @override
  bool get isSupported =>
      false; // USB requires platform-specific implementation
...
class LedgerBLETransport implements LedgerTransport {
  ...
  @override
  bool get isSupported =>
      false; // BLE requires platform-specific implementation
...
class MockLedgerTransport implements LedgerTransport {
  ...
  void setMockResponse(APDUCommand command, Uint8List responseData,
      {int statusWord = 0x9000}) {
    final key = _commandKey(command);
    final response = Uint8List.fromList(
        [...responseData, (statusWord >> 8) & 0xFF, statusWord & 0xFF]);
