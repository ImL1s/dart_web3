...
class AnimatedQR {
  ...
  static AnimatedQR fromEncodedParts(List<String> parts, {Duration? interval}) {
    return AnimatedQR(parts,
        interval: interval ?? const Duration(milliseconds: 500));
  }

  /// Create an animated QR by encoding data
  static AnimatedQR fromData(
    String type,
    Uint8List data, {
    int? fragmentLength,
    Duration? interval,
    ...
    final parts =
        BCUREncoder.encodeMultiple(type, data, fragmentLength: fragmentLength);
    return AnimatedQR(parts,
        interval: interval ?? const Duration(milliseconds: 500));
  }
}
