...
class QRCommunication {
  ...
  final StreamController<QRCommunicationState> _stateController = StreamController.broadcast();
  final StreamController<String> _qrDisplayController = StreamController.broadcast();
  final StreamController<QRProgress> _progressController = StreamController.broadcast();
  ...
  Future<void> displaySignRequest(KeystoneSignRequest request) async {
    ...
    try {
      ...
      final parts = [encoded]; // Single part for now, can be extended for large data
      ...
      _handleError(KeystoneErrorType.qrCodeError, 'Failed to display sign request: $e');
    }
  }
  ...
  Future<KeystoneSignResponse?> processScannedQR(String qrData) async {
    ...
    try {
      ...
      _handleError(KeystoneErrorType.qrCodeError, 'Failed to process QR response: $e');
      return null;
    }
  }
...
class MockQRScanner implements QRScanner {
  final StreamController<QRScanResult> _controller = StreamController.broadcast();
