  ...
  group('Solana Vector Tests', () {
    ...
    for (var i = 0; i < vectors.length; i++) {
      ...
      test('Vector #$i: $description', () {
        ...
        final actualHeaderHex =
            HexUtils.encode(serialized.sublist(0, 3), prefix: false);
        expect(actualHeaderHex, equals(expectedHeaderHex),
            reason: 'Header mismatch');

        if (vector.containsKey('hex')) {
          expect(HexUtils.encode(serialized, prefix: false),
              equals(vector['hex'] as String));
        }
      });
    }
    ...
    group('Borsh Encoding Vectors', () {
      test('PublicKey Borsh Serialization', () {
        final pubKey = PublicKey.fromString(
            '4u68Abtp6YF34T8uKktS9kFidQByx2hTFYJb5yF8KPBn');
