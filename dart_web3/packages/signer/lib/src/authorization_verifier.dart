...
class AuthorizationVerifier {
  ...
  static bool verifyAuthorization(Authorization authorization, String expectedSigner) {
  ...
  static Map<int, bool> verifyAuthorizationList(
    ...
    if (authorizations.length != expectedSigners.length) {
      throw ArgumentError('Authorization list and signer list must have the same length');
    }
  ...
  static bool isValidAuthorizationFormat(Authorization authorization) {
    try {
      ...
      if (authorization.isSigned) {
        ...
        if (authorization.r >= secp256k1Order || authorization.s >= secp256k1Order) return false;
      }
  ...
  static bool isValidAuthorizationListFormat(List<Authorization> authorizations) {
  ...
  static String? recoverSigner(Authorization authorization) {
    ...
    try {
      ...
      final recoveredPublicKey = Secp256k1.recover(signature, messageHash, authorization.yParity);
